...
const createUser= async(req,res)=>{
    try{
        ...
        const insertQuery="INSERT INTO user (UserID,Username,Password,UserType) VALUES (?,?,?,?)"
...
const updateUser=async(req,res)=>{
    try{
        ...
        const updateQuery="UPDATE users SET Username=?, Password=?, UserType=? where UserID=?";
...
const deleteUser= async(req,res)=>{
    try{
        ...
        if(!userId || typeof UserID!=="string"){
            return res
            .status(400).json({error:"Invalid User ID"})
        }
